...
#------------------------------------------------------
quietly set fp [open "$source_path/scripts/compile_order.txt" r]
...
foreach item $file_data {
  if {$idx > 2} {
    echo "eval vcom  $compdirectives  $source_path/scripts/$item"
    eval vcom  $compdirectives  $source_path/scripts/$item
  }
  incr idx 1
}